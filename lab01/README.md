...
Шпаргалочка:
1) git add . - подготовка с сохранению 
2) git status - что готово к сохранению
3) git commit -m 'jxj' - сохранить с описанием (jxj)
4) git push - отправить изменения в облако
5) git pull - забрать изменения с облака
